namespace EroMangaManager.WinUI3.Helpers
{
    ...
    public static class CoverHelper
    {
        ...
        /// <returns> </returns>
        public static async Task<string> TryCreatCoverFileAsync (string storageFile , FilteredImage[] filteredImages)
        {
            string path;
        ...
        /// <returns></returns>
        public static async Task<string> CreatCoverFile_Origin_SharpCompress (this string storageFile , FilteredImage[] filteredImages)
        {
            string path = null;
            ...
            try
            {
                using (var zipArchive = ArchiveFactory.Open(stream))
                {
                    foreach (var entry in zipArchive.Entries)
                    {
                        ...
                        if (canuse)
                        {
                            path = Path.Combine(coverfolder.Path , Path.GetFileNameWithoutExtension(storageFile) + ".jpg");
